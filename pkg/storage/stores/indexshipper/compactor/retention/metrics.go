...
type CleanupMetrics struct {
	...
	chunksProcessedDurationSeconds *prometheus.HistogramVec
}

func NewCleanupMetrics(r prometheus.Registerer) *CleanupMetrics {
	return &CleanupMetrics{
		chunksProcessedTotal: promauto.With(r).NewCounterVec(
		...
		chunksDeletedTotal: promauto.With(r).NewCounterVec(
			prometheus.CounterOpts{
				Namespace: "loki_boltdb_shipper",
				Name:      "retention_cleanup_chunks_deleted_count_total",
				Help:      "Total count of chunks deleted per table.",
			}, []string{"table", "user_id", "action"}),
		chunksProcessedDurationSeconds: promauto.With(r).NewHistogramVec(
			prometheus.HistogramOpts{
				Namespace: "loki_boltdb_shipper",
				Name:      "retention_cleanup_chunks_processed_duration_seconds",
				Help:      "Time (in seconds) spent in processing table chunks",
				Buckets:   []float64{1, 2.4, 5, 10, 20, 40, 90, 360, 600, 1800},
			}, []string{"table", "status"}),
	}
}
